...
你也可以自定义加密解密规则，当然无论是否加密，我都不建议在`custom.js`文件中填写`apiKey`，有心之人防不住，容易泄露！这只是为了防止`github`识别的方案，建议直接部署后在首页填入自己的`apiKey`使用。如需对外提供服务，请使用项目后端版本，因为此项目前端版本为纯静态，对外开放按上述方案依旧会使得Key泄露，感兴趣的可贡献node后端，使用Vercel也可更好的零成本部署。总之纯前端版本流式响应效果不好，需要速度快点的建议使用后端版本或者用 Vercel 代替github pages!
